...
var actions = {
  ...
  SHOW_CONNECT_PAGE: 'SHOW_CONNECT_PAGE',
...
function connectHardware (deviceName) {
  log.debug(`background.connectHardware`, deviceName)
  return (dispatch, getState) => {
    dispatch(actions.showLoadingIndication())
    return new Promise((resolve, reject) => {
      background.connectHardware(deviceName, (err, accounts) => {
        if (err) {
          log.error(err)
          dispatch(actions.displayWarning(err.message))
          return reject(err)
...
function showConnectPage () {
  return {
    type: actions.SHOW_CONNECT_PAGE,
  }
}
